...
class FBMSNet(nn.Module):
    ...
    def LastBlock(self, inF, outF, doWeightNorm=True, *args, **kwargs):
        return nn.Sequential(
            ...
            nn.LogSoftmax(dim=1))
    def __init__(self, nChan, nTime, nClass=2, temporalLayer='LogVarLayer', num_Feat=36, dilatability=8, dropoutP=0.5, *args, **kwargs):
    ...
    def forward(self, x):
        ...
        x = self.scb(x)
        Time = x.shape[3]
        T_crop = (Time // self.strideFactor) * self.strideFactor
        x = x[..., :T_crop]  # 裁切成可以整除的長度
        x = x.reshape([*x.shape[0:2], self.strideFactor, int(T_crop / self.strideFactor)])
